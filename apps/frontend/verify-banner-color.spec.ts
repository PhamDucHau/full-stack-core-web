...
test.describe('Home Page Banner Verification', () => {
  test('banner section should have cyan/ocean blue background color', async ({ page }) => {
    ...
    // Get the computed background color or check if it has cyan classes
    const className = await heroSection.getAttribute('class');

    // Verify the class contains cyan/ocean blue background color
    ...
    console.log('✅ Banner color verification passed: Cyan/ocean blue background class is present');
  });
});
