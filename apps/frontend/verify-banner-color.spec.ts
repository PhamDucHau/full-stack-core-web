...
test.describe('Home Page Banner Verification', () => {
  test('banner section should have yellow background color', async ({ page }) => {
    ...
    // Verify the class contains yellow gradient colors
    expect(className).toContain('from-yellow-500');
    expect(className).toContain('via-yellow-600');
    expect(className).toContain('to-yellow-700');
    ...
    console.log('✅ Banner color verification passed: Yellow gradient classes are present');
  });
});
